package cn.mall.shop.module.infra.service.codegen.inner;
...
import cn.mall.shop.framework.apilog.core.annotation.ApiAccessLog;
import cn.mall.shop.framework.apilog.core.enums.OperateTypeEnum;
import cn.mall.shop.framework.common.exception.util.ServiceExceptionUtil;
...
import cn.mall.shop.framework.common.util.date.DateUtils;
...
import cn.mall.shop.framework.excel.core.annotations.DictFormat;
import cn.mall.shop.framework.excel.core.convert.DictConvert;
import cn.mall.shop.framework.excel.core.util.ExcelUtils;
import cn.mall.shop.framework.mybatis.core.dataobject.BaseDO;
...
import cn.mall.shop.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 shop-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 shop-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.mall.shop.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/shop-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/shop-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "shop-module-${table.moduleName}/" + // 顶级模块
                "shop-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "shop-module-${table.moduleName}/" + // 顶级模块
                "shop-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "shop-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "shop-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "shop-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
